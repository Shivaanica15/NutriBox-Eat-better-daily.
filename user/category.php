...
<head>
   ...
   <title>meal plans by diet</title>
...
<section class="p-category">
   ...
   <?php
   $diet_links = $conn->prepare("SELECT DISTINCT diet_type FROM `meal_plans`");
   $diet_links->execute();
   if ($diet_links->rowCount() > 0) {
       while ($diet = $diet_links->fetch(PDO::FETCH_ASSOC)) { ?>
      <a href="category.php?diet_type=<?= $diet["diet_type"] ?>"><?= $diet[
    "diet_type"
] ?></a>
   <?php }
   }
...
<section class="products">
   ...
   <h1 class="title">meal plans by diet type</h1>
   ...
   if ($diet_type !== "") {
       $select_plans = $conn->prepare(
           "SELECT * FROM `meal_plans` WHERE diet_type = ?",
       );
       $select_plans->execute([$diet_type]);
   } else {
   ...
   <?php }
       ...
       echo '<p class="empty">no meal plans available!</p>';
   }
   ?>
