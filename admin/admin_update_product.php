...
if (isset($_POST["update_meal_plan"])) {
    ...
    if (
        ...
        $update_plan = $conn->prepare(
            "UPDATE `meal_plans` SET name = ?, description = ?, price = ?, duration = ?, calories = ?, diet_type = ? WHERE id = ?",
        );
        $update_plan->execute([
            $name,
            $description,
        ...
        if (!empty($image)) {
            if ($image_size > 2000000) {
                ...
                $update_image = $conn->prepare(
                    "UPDATE `meal_plans` SET image = ? WHERE id = ?",
                );
...
<section class="update-product">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <input type="text" name="name" placeholder="enter meal plan name" required class="box" value="<?= $fetch_plan[
          "name"
      ] ?>">
      <input type="text" name="diet_type" placeholder="enter diet type" required class="box" value="<?= $fetch_plan[
          "diet_type"
      ] ?>">
      <input type="number" name="price" min="0" placeholder="enter price" required class="box" value="<?= $fetch_plan[
          "price"
      ] ?>">
      <input type="number" name="duration" min="1" placeholder="enter duration (days)" required class="box" value="<?= $fetch_plan[
          "duration"
      ] ?>">
      <input type="number" name="calories" min="0" placeholder="enter calories per day" required class="box" value="<?= $fetch_plan[
          "calories"
      ] ?>">
      <textarea name="description" required placeholder="enter meal plan description" class="box" cols="30" rows="10"><?= $fetch_plan[
          "description"
      ] ?></textarea>
