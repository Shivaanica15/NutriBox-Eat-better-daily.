...
if (isset($_GET["delete"])) {
    ...
    $select_delete_image = $conn->prepare(
        "SELECT image FROM `meal_plans` WHERE id = ?",
    );
    ...
    if (
        ...
        file_exists("../uploaded_img/" . $fetch_delete_image["image"])
    ) {
    ...
    $delete_wishlist = $conn->prepare(
        "DELETE FROM `wishlist` WHERE meal_plan_id = ?",
    );
...
<section class="show-products">
   ...
   <div class="box">
      ...
      <div class="flex-btn">
         <a href="admin_update_product.php?update=<?= $fetch_plan[
             "id"
         ] ?>" class="option-btn">update</a>
         <a href="admin_products.php?delete=<?= $fetch_plan[
             "id"
         ] ?>" class="delete-btn" onclick="return confirm('delete this meal plan?');">delete</a>
      </div>
   </div>
   <?php }
   } else {
